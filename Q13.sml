...
fun w c (Int(_)) = ([], T_Int)
  ...
  | w c (If(x,y,z)) = 
    let 
      ...
      val s4 = unify t2 t3
    in
      (s4 o s3 o s2 o s1, (subExp t2 s4)) 
    end
  | w c (UnOp(x,y)) =
      let
        val (s,t) = w c y
  ...
  | w c (BinOp(x,y,z)) =
      ...
      in
        (s4 o s3 o s2 o s1, (subExp t1 s3)) 
      end
...
resetTVar();
(* check_expect 27 (typeExpr (BinOp(OpTimes, (Fun("x", Var("x"))), Fun("y", Var("y")))))
                (T_Fun(T_Var(2), T_Var(2)));
resetTVar();
check_expect 28 (typeExpr (BinOp(OpTimes, (Fun("x", Var("x"))), Fun("y", Int(1)))))
                (T_Fun(T_Int, T_Int));*)
